...
export async function recordKinloopApproval({ client, option } = {}) {
  const userId = await resolveUserId(client);
  ...
  if (giftResult.error || !giftResult.data) {
    return recordKinloopAuditEvent({
      ...
      summary: `${option.title} approved for Sarah.`,
      metadata: {
        gift_id: option.id,
        title: option.title,
  ...
  if (approvalResult.error || auditResult.error) {
    return {
      mode: "local",
      approvedGiftId: option.id,
      fallbackReason: approvalResult.error?.message || auditResult.error?.message
    };
  }
