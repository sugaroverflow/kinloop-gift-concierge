...
function validateGiftOptions(payload, products = gifts) {
  ...
  const validated = rawOptions.slice(0, 3).map((candidate) => validateGiftSourceCandidate(candidate, products));
...
function validateGiftSourceCandidate(candidate, products = gifts) {
  const fallback = products[0] || gifts[0];
