...
async function generateCodexGiftOptions({ input, person, products }) {
  ...
  try {
    ...
    return validateGiftOptions(JSON.parse(turn.finalResponse), { products, person, input });
  } finally {
...
function validateGiftSourceCandidate(candidate, { products = gifts, index = 0, person = recipients[0], input = "" } = {}) {
  ...
  const relationalCaption = `${safeText(fallback.caption, "Gift details available before approval.")} Picked for ${personFirstName}'s ${clueText} context.`;
  const relationalWhy = `Connects ${personFirstName}'s ${clueText} clues with ${safeText(fallback.displayPrice, "the budget range")} and the ${safeText(person?.birthday, "birthday")} timeline.`;
  ...
  return {
    ...
    priceRange: shouldRewritePriceText(candidate?.priceRange)
      ? safeText(fallback.displayPrice, "Price confirmed at checkout")
      : safeText(candidate?.priceRange, safeText(fallback.displayPrice, "Price confirmed at checkout")),
...
function shouldRewritePriceText(value) {
  const text = textFromUnknown(value).toLowerCase();
  if (!text) return true;
  return text.includes("price shown") || text.includes("merchant checkout");
}
