...
export function normalizeGiftSourceRequest(raw = {}) {
  ...
  const resolvedPersonId = briefPersonId || stringOrEmpty(body.personId) || sourcePersonId || "sarah";
  const signalMatchesPerson = !sourcePersonId || sourcePersonId === resolvedPersonId;
  ...
  const clues = uniqueClean([
    ...
    ...(signalMatchesPerson ? arrayOfStrings(extracted.interests) : []),
    ...(basePerson.likes || [])
  ]).slice(0, 8);
  const avoid = uniqueClean([
    ...arrayOfStrings(brief.avoid),
    ...arrayOfStrings(signalMatchesPerson ? extracted.avoid : []),
    ...(basePerson.avoid || [])
  ]).slice(0, 8);
  const sourceText = normalizeSourceInput([
    stringOrEmpty(body.input),
    ...(signalMatchesPerson ? [stringOrEmpty(sourceSignal.sourceText)] : []),
    stringOrEmpty(brief.sourceText),
    signalMatchesPerson && stringOrEmpty(extracted.giftLead) ? `Gift lead: ${stringOrEmpty(extracted.giftLead)}` : "",
  ...
  const person = {
    ...
    budget: (signalMatchesPerson ? stringOrEmpty(extracted.budget) : "") || stringOrEmpty(brief.budget) || basePerson.budget,
    addressStatus: stringOrEmpty(brief.addressStatus) || basePerson.addressStatus,
    note: stringOrEmpty(brief.note) || (signalMatchesPerson ? stringOrEmpty(extracted.giftLead) : "") || basePerson.note,
