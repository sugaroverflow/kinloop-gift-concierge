  ...
  if (preferLive && hasShopifyCatalogConfig() && typeof fetchImpl === "function") {
    try {
      ...
      return {
        ...
        fallbackReason: "Shopify catalog returned no products.",
...
export async function fetchShopifyCatalogProducts({ input = "", person = recipients[0], limit = 12, fetchImpl = globalThis.fetch } = {}) {
  ...
  const products = extractCatalogProducts(payload).slice(0, limit * 2).map(normalizeCatalogProduct);
  return dedupeCatalogProducts(products.filter((product) => product.id && product.name)).slice(0, limit);
}
...
function normalizeCatalogProduct(product) {
  ...
  return {
    ...
    delivery: product.delivery || product.delivery_label || product.shipping?.summary || "Confirm delivery before approving",
    seller,
...
function resolvePrice(product) {
  ...
  return {
    amount: 0,
    label: product.displayPrice || product.price_label || "Confirm price before approving"
  };
}
...
function dedupeCatalogProducts(products) {
  ...
  for (const product of products) {
    const key = String(product.name || product.id).toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
